...
bool parallel_for_range::getgrain( const unsigned int nCount,const unsigned int nCores, const parallel_for_range::graintype gt, unsigned int& nGrain )
{
	...
	switch(gt)
	{
		case gt_floor:nGrain = std::max<unsigned int>(1,af::posfloor<double>(nCount/static_cast<double>(nCores)));break;
		case gt_round:nGrain = std::max<unsigned int>(1,af::posround<double>(nCount/static_cast<double>(nCores)));break;
		case gt_ceil:nGrain = std::max<unsigned int>(1,af::posceil<double>(nCount/static_cast<double>(nCores)));break;
...
void parallel_for_range::getgrain( const unsigned int nFrom,const unsigned int nCount,const unsigned int nCores, const graintype gt )
{
	...
	switch(gt)
	{
		case gt_floor:m_nWholeSubRangeSize = std::max<unsigned int>(1,af::posfloor<double>(nCount/static_cast<double>(nCores)));break;
		case gt_round:m_nWholeSubRangeSize = std::max<unsigned int>(1,af::posround<double>(nCount/static_cast<double>(nCores)));break;
		case gt_ceil:m_nWholeSubRangeSize = std::max<unsigned int>(1,af::posceil<double>(nCount/static_cast<double>(nCores)));break;

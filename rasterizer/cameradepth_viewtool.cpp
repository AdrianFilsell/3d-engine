...
bool cameradepth_viewtool::render(void) const
{
	return m_pView && !!m_pView->getselection();
}

void cameradepth_viewtool::selectionchanged( void )
{
}

void cameradepth_viewtool::render(afdib::dib *pDst,const af3d::rect& rDevice,const surface *pSurface)const
{
	renderbbox(pDst,rDevice,pSurface,0);
}

void cameradepth_viewtool::getviewht(const af3d::vec2<int>& rtpt,std::shared_ptr<hittest<>>& ht )const
{
	ht=nullptr;
}

void cameradepth_viewtool::cancel(void)
{
...
void cameradepth_viewtool::begindrag( const af3d::vec2<int>& rtptStart, const af3d::vec2<int>& rtptMove )
{
	af3d::mat4<> mProj;
	...
	m_DepthWorldPlaneBasis=af3d::planebasis<>(m_pView->getsurface()->getcamera()->getorigin(),
											  m_pView->getsurface()->getcamera()->getup().cross(worlddir/*m_pView->getsurface()->getcamera()->getdir()*/),
...
void cameradepth_viewtool::movedrag( const af3d::vec2<int>& rtpt )
{
	translate(rtpt);
}

void cameradepth_viewtool::enddrag( const af3d::vec2<int>& rtpt )
{
	translate(rtpt);
...
void cameradepth_viewtool::translate( const af3d::vec2<int>& rtpt )
{
